...
class DRN(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.out_map:
            ...
            pred = self.fc(F.interpolate(y[-3], size=(512, 512))) + self.fc(F.interpolate(y[-2], size=(512, 512))) + self.fc(F.interpolate(y[-1], size=(512, 512)))
...
def drn_c_26(pretrained=False, **kwargs):
    ...
    if pretrained: 
        vgg_state_dict = model_zoo.load_url(model_urls['drn-c-26'])
...
def drn_d_54(pretrained=False, **kwargs):
    ...
    if pretrained: 
        vgg_state_dict = model_zoo.load_url(model_urls['drn-d-54'])
        yolo_state_dict = model.state_dict()
        for k in vgg_state_dict.keys():
            if k != 'fc.weight' and k != 'fc.bias':
                yolo_state_dict[k] = vgg_state_dict[k]
...
def drn_d_56(pretrained=False, **kwargs):
    ...
    if pretrained: 
        vgg_state_dict = model_zoo.load_url(model_urls['drn-d-56'])
        yolo_state_dict = model.state_dict()
        for k in vgg_state_dict.keys():
            try:
                yolo_state_dict[k] = vgg_state_dict[k]
